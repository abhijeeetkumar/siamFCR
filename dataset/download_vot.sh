...
(
    ...
    cat description.json | ../../jq-linux64 -r '.sequences[] | .annotations.url' >annotations.txt
    cat description.json | ../../jq-linux64 -r '.sequences[] | .channels.color.url' >color.txt
    cat description.json | ../../jq-linux64 -r '.sequences[] | .name' >list.txt
    mkdir -p annotations
    (
        cd annotations
